...
| `refresh_token_origin` | The full origin (`scheme://host:port`) of the API base URL that issued the current `refresh_token`, e.g. `https://api.example.com`. Restores the host-only scoping a real cookie would have had (`cookies.ts` sets no `Domain` attribute, so the cookie the backend issues is already host-only — this key exists purely to replicate that boundary, since the manual mimicry has no transport-level equivalent). | Same as `refresh_token` | Same as `refresh_token` |
| `refresh_token_expires_at` | The `Max-Age`/`Expires` value from the same `Set-Cookie`, converted to an absolute epoch timestamp. Normally equal to `getRefreshTokenTtlMs()` (14 days), but tracked independently because `backend/src/auth/cookies.ts` allows `REFRESH_COOKIE_MAX_AGE_MS` to be configured shorter than the DB-side TTL (not currently set anywhere in this project's `.env`/`docker-compose*.yml`, but the code path exists) — a native client that ignores it would keep sending a token past the point a browser would have already dropped the cookie. | Same as `refresh_token` | Same as `refresh_token` |
| `access_token` | Not persisted. Keep in memory only (Riverpod state); re-derive via `/auth/refresh` on cold start using the persisted `refresh_token`. Minimizes on-disk exposure window given the 15-minute lifetime already makes persistence low-value. | — | App restart (by construction, since it's memory-only), **and explicitly on logout and on any `400`/`401` from `/auth/refresh`** — `authController.logout` can only revoke the refresh token; it cannot revoke an already-issued stateless JWT access token, so if the in-memory value is left alone after logout, code running in the same still-open app process could keep calling protected endpoints with it for up to its remaining 15-minute lifetime. Logout and refresh-rejection must clear this value and drive `AuthNotifier` to `unauthenticated` in the same step, not rely on process exit. |

**Three attributes must be persisted alongside the token and re-checked on every send, not only at write time** (this generalizes the `Secure`-only version of this rule from an earlier revision of this document to cover host binding and expiry as well, per review feedback):

1. **`Secure`.** `backend/src/auth/cookies.ts` only omits `Secure` when `NODE_ENV` is `development`/`test` — in any other environment (i.e. real production) the cookie is marked `Secure` specifically so it is never sent over plain HTTP. The app's configured base URL can change after the token was stored (an app update, a build flavor swap, or a user manually pointed at a different backend), so this must be re-validated against the *current* base URL scheme on every `onRequest`: if `refresh_token_secure` is `"1"` and the current scheme is not `https`, refuse to attach the cookie and treat the stored token as invalid.
2. **Issuing origin (`refresh_token_origin`).** A real browser cookie is host-only by default (no `Domain` attribute is set in `cookies.ts`), so a cookie issued by `api-a.example` is never sent to `api-b.example`. The manual mimicry has no equivalent unless it checks this explicitly — otherwise switching the configured base URL from one HTTPS host to another HTTPS host would silently replay a token issued by the first host against the second. On every `onRequest`, compare the current base URL's origin against `refresh_token_origin`; on any mismatch, refuse to attach the cookie and treat the stored token as invalid.
3. **Expiry (`refresh_token_expires_at`).** Compare against the current time on every `onRequest`; if past, treat the same as an invalid token (skip attaching the cookie, let the request 401 normally, or proactively clear and force re-login).

In all three cases the failure handling is the same: refuse to attach the `Cookie` header, delete the four `refresh_token*` keys, and drive `AuthNotifier` to `unauthenticated` — never silently degrade by sending the token anyway or by silently dropping a legitimately valid token.

Outgoing request handling (`dio` interceptor, for #332):
- On `onRequest` for `POST /auth/refresh` and `POST /auth/logout`: read all four `refresh_token*` keys from secure storage, re-validate `Secure`/origin/expiry against the current base URL and clock as described above, and if all three pass, set request header `Cookie: refresh_token=<value>`. Logout must attach it too — `authController.logout` only calls `revokeToken` when it can read the cookie from the request, so skipping this on logout would clear local storage while leaving the token valid server-side until it naturally expires (up to 14 days later).
- On `onResponse` for `/auth/register`, `/auth/login`, `/auth/refresh`: read the `set-cookie` response header, extract the `refresh_token=<value>` segment plus its `Secure` flag and `Max-Age`/`Expires`, and write the value and all three derived attributes to secure storage only if the `Secure` check above passes (overwriting the previous values — the token rotates every call).
- On a `400`/`401` from `/auth/refresh`: delete all four stored `refresh_token*` keys and the in-memory `access_token`, and drive `AuthNotifier` to `unauthenticated` (#333's session-restore flow should treat "no stored refresh_token" and "refresh call rejected" identically).
