...
- On `onRequest` for `POST /auth/refresh` specifically: read `refresh_token` from secure storage and set request header `Cookie: refresh_token=<value>`.
- On `onResponse` for `/auth/register`, `/auth/login`, `/auth/refresh`: read the `set-cookie` response header, extract the `refresh_token=<value>` segment, write it to secure storage (overwriting the previous value — the token rotates every call).
