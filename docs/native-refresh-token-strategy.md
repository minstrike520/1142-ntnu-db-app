...
**Decision (TL;DR): Keep the existing cookie-based `/auth/refresh` contract unchanged (Option A, "cookie mimicry"). Native clients do not use a generic cookie-jar library; instead a `dio` interceptor extracts the `refresh_token` value from the raw `Set-Cookie` response header on `/auth/register`, `/auth/login`, and `/auth/refresh`, stores only that value in `flutter_secure_storage` (honoring the `Secure` attribute rather than ignoring it), and manually attaches it as a `Cookie: refresh_token=<value>` request header on `/auth/refresh` and `/auth/logout` calls, with refresh calls serialized to a single in-flight request. No backend API contract change is required.**
...
**`Secure` attribute must still be honored, not ignored.** `backend/src/auth/cookies.ts` only omits `Secure` when `NODE_ENV` is `development`/`test` — in any other environment (i.e. real production) the cookie is marked `Secure` specifically so it is never sent over plain HTTP. A native client that stores and resends the token regardless of this attribute defeats that guard the moment someone points a release build at a plain-HTTP host. The interceptor must therefore parse the `Secure` flag off the `Set-Cookie` header and enforce it: if `Secure` is present, only persist/resend the token when the API base URL scheme is `https`; if the base URL is `http` and the cookie is `Secure`, refuse to store it and surface an error rather than silently degrading (this only exempts the project's own `NODE_ENV=development`/`test` backends, which is exactly the case that omits `Secure` in the first place — no separate client-side dev override flag is needed).

Outgoing request handling (`dio` interceptor, for #332):
- On `onRequest` for `POST /auth/refresh` and `POST /auth/logout`: read `refresh_token` from secure storage and set request header `Cookie: refresh_token=<value>`. Logout must attach it too — `authController.logout` only calls `revokeToken` when it can read the cookie from the request, so skipping this on logout would clear local storage while leaving the token valid server-side until it naturally expires (up to 14 days later).
- On `onResponse` for `/auth/register`, `/auth/login`, `/auth/refresh`: read the `set-cookie` response header, extract the `refresh_token=<value>` segment and the `Secure` flag, and write the value to secure storage only if the `Secure` check above passes (overwriting the previous value — the token rotates every call).
- On a `400`/`401` from `/auth/refresh`: delete the stored `refresh_token` and drive `AuthNotifier` to `unauthenticated` (#333's session-restore flow should treat "no stored refresh_token" and "refresh call rejected" identically).
