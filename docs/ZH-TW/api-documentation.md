...
| `user_status` | `{ userId: string, status: 'online' \| 'offline' }` | 好友的上線 / 下線狀態更新。於好友連線或斷線時推送。事件會送往每位好友的 `user_<id>` room，因此能送達他們在任一 instance 上的 session。每次狀態轉換只會由取得第一個 lease 或釋放最後一個 lease 的那個 instance 對整個 cluster 宣告一次。 |
