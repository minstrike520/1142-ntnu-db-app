...
- **若上述成功路徑中任一環節失敗，必須丟棄舊記錄，不可保留。** `/auth/refresh` 回傳 `200` 即代表伺服器已提交輪替，因此自該刻起，先前儲存的權杖已被撤銷且帶有 `replacedBy`，無論用戶端接下來做了什麼都一樣。若 `Set-Cookie` Header 缺失或無法解析、新值未通過 `Secure`／origin／到期時間檢查，或 `write()` 本身拋出例外（儲存空間不足、keychain 被鎖定、權限錯誤），用戶端都必須刪除 `refresh_token_record`、清除記憶體中的 `access_token`，並將 `AuthNotifier` 導向 `unauthenticated`，強制重新登入。保留舊記錄以期後續重試只會更糟：下次使用它時（無論是本次執行或冷啟動後）都會呈現一個已輪替失效的權杖，觸發 `revokeAllForUser`，使該使用者**所有裝置**都被登出，而不只是這一台。此處代價明顯不對稱（一次重新登入 vs. 所有工作階段被撤銷），因此此路徑上的失敗一律必須退回重新登入。請注意與下方限制 1 的差異：限制 1 中用戶端根本不知道輪替已發生，因此無從處理；而此處的 `200` 正是輪替已發生的證據，這使得丟棄舊權杖既為可行、也屬必要。
