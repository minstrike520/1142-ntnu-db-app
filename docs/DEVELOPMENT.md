...
4. **CI → Stack Hand-off**: `.github/workflows/release-bridge.yml` listens for that same `ci.yml` run completing, finds the `vX.Y.Z` tag (on the merge commit or on the version commit created during the run), and dispatches `release-stack.yml`. This bridge exists because anything pushed with the default `GITHUB_TOKEN` — tag *or* commit — does not trigger a workflow run. That is also why the version commit never gets a CI run of its own, and why `release-stack.yml` accepts a successful CI run for the tag commit **or its first parent**.
