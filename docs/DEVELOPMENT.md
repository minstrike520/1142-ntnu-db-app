...
4. **CI → Stack Hand-off**: The release job creates a short-lived token for the dedicated Release GitHub App, whose installation and tag-ruleset bypass allow Semantic Release to push the protected `vX.Y.Z` tag. App pushes start workflows, so the version commit gets a second `CI` run. To avoid duplicate or racing publications, `release-stack.yml` does not listen for tag pushes; `.github/workflows/release-bridge.yml` waits for the producing `ci.yml` run, finds the tag, and dispatches Stack publication exactly once while ignoring the version commit's bridge run.
