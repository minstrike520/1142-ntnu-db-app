...
-- Restore the views owned by 2026061501000 so a rollback leaves the previous
-- application version usable instead of leaving its repository queries without
-- the relations they expect.
CREATE OR REPLACE VIEW message_with_sender_view AS
SELECT
  m.message_id,
  m.room_id,
