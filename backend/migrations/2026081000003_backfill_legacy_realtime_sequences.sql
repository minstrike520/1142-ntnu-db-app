...
-- invalidate cursors held by clients that connected during the rollout.
...
-- Preserve the historical visibility/read state while changing the sequence
-- values. Recompute rather than taking GREATEST: the old appended numbering
-- may be larger than the rebuilt room-local numbering.
...
-- Down migration intentionally leaves the assigned ordering intact: reverting
-- durable ordering would make already-issued cursors ambiguous.
--- Down migration
SELECT 1;
